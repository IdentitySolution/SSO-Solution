...
// Modifications Copyright (C) 2019 Daniel Carrera
...
namespace TemplateSourceName
{
    ...
    {
        public static void EnsureCleanData(IServiceProvider provider)
        {
            provider.GetRequiredService<ApplicationDbContext>().Database.EnsureDeleted();
            provider.GetRequiredService<PersistedGrantDbContext>().Database.EnsureDeleted();
            provider.GetRequiredService<ConfigurationDbContext>().Database.EnsureDeleted();
        }
        ...
        public static void EnsureSeedData(IServiceProvider provider)
        {
...

            {
                ...
                if (alice == null)
                {
                    ...
                    alice = userMgr.FindByNameAsync(alice.UserName).Result;
                ...
                if (bob == null)
                {
                    ...
                    bob = userMgr.FindByNameAsync(bob.Email).Result;
